...
async def __call():
    rsp = await AioImageSynthesis.call(api_key=os.getenv("DASHSCOPE_API_KEY"),
                                 model=model,
                                 prompt=prompt,
                                 n=1,
                                 size='1024*1024')
...
async def __async_call():
    rsp = await AioImageSynthesis.async_call(api_key=os.getenv("DASHSCOPE_API_KEY"),
                                 model=model,
                                 prompt=prompt,
                                 n=1,
                                 size='1024*1024')
...
async def __sync_call():
    ...
    """
    rsp = await AioImageSynthesis.sync_call(api_key=os.getenv("DASHSCOPE_API_KEY"),
                                 model=model,
                                 prompt=prompt,
                                 n=1,
                                 size='1024*1024')
