...
- :mod:`~dashscope.finetune.reinforcement.component.observability.genai.messages` — message and tool_calls mapping
- :mod:`~dashscope.finetune.reinforcement.component.observability.genai.llm_openai` — OpenAI-compatible LLM client patching
- :mod:`~dashscope.finetune.reinforcement.component.observability.genai.llm_dashscope` — DashScope Generation patching
- :mod:`~dashscope.finetune.reinforcement.component.observability.genai.tools` — tool tracing (decorator + patcher)
...
from dashscope.finetune.reinforcement.component.observability.genai.llm_dashscope import (
    instrument_dashscope_generation_call,
)
from dashscope.finetune.reinforcement.component.observability.genai.llm_openai import (
    instrument_openai_chat_completions,
)
from dashscope.finetune.reinforcement.component.observability.genai.llm_span import (
    genai_llm_span,
    observe_llm,
...
from dashscope.finetune.reinforcement.component.observability.genai.tools import (
    observe_tool,
    trace_tool,
)
...
def trace_client(client: Any) -> None:
    ...
    * **Full OpenAI client** — ``openai.OpenAI`` / ``openai.AsyncOpenAI`` instances
      that expose ``.chat.completions.create``.
    * **Completions resource** — objects that have a ``.create`` method but no
      ``.chat`` attribute (e.g. ``ChatOpenAI.client`` from ``langchain-openai``).
    * **LangChain-like LLM wrapper** — objects with ``.client`` and/or ``.async_client``
      attributes that point to completions resources. Both sync and async resources
      are patched if present.
    * **DashScope Generation class** — the class itself (not an instance), which
      has a ``call`` classmethod.
    ...
    # 3. LangChain-like wrapper: has .client and/or .async_client pointing to completions
    ...
    has_call_classmethod = _inspect.isclass(candidate) and isinstance(
        candidate.__dict__.get("call"), classmethod
    )
