...
from dashscope.agentstudio.resources.session_events import SessionEvents, AsyncSessionEvents
from dashscope.agentstudio.types import DeleteResponse, Session
from dashscope.agentstudio.types.params import SessionCreateParams, SessionListParams, SessionUpdateParams
...
class Sessions:
    ...
    def create(
        ...
        resp = self._client._transport.request("POST", _PATH_SESSIONS, json=body)
        return _coerce_session(resp.data)

    def retrieve(self, session_id: str) -> Session:
        resp = self._client._transport.request("GET", f"{_PATH_SESSIONS}/{session_id}")
        return _coerce_session(resp.data)
    ...
    def update(
        ...
        resp = self._client._transport.request("POST", f"{_PATH_SESSIONS}/{session_id}", json=body)
        return _coerce_session(resp.data)
    ...
    def list(
        ...
        params = SessionListParams(
            limit=limit, page=page, agent_id=agent_id, statuses=statuses,
            created_at_gt=created_at_gt, created_at_gte=created_at_gte,
            created_at_lt=created_at_lt, created_at_lte=created_at_lte,
        ).to_dict()
        resp = self._client._transport.request("GET", _PATH_SESSIONS, params=params)

        def fetch_next(nxt: str) -> CursorPage[Session]:
            return self.list(
                limit=limit, page=nxt, agent_id=agent_id, statuses=statuses,
                created_at_gt=created_at_gt, created_at_gte=created_at_gte,
                created_at_lt=created_at_lt, created_at_lte=created_at_lte,
            )

        return build_page(
            payload=resp.data, item_factory=_coerce_session, request_id=resp.request_id,
            fetch_next=fetch_next,
        )

    def archive(self, session_id: str) -> Session:
        resp = self._client._transport.request("POST", f"{_PATH_SESSIONS}/{session_id}/archive")
        return _coerce_session(resp.data)

    def delete(self, session_id: str) -> DeleteResponse:
        resp = self._client._transport.request("DELETE", f"{_PATH_SESSIONS}/{session_id}")
...
class AsyncSessions:
    ...
    async def create(
        ...
        resp = await self._client._transport.request("POST", _PATH_SESSIONS, json=body)
        return _coerce_session(resp.data)

    async def retrieve(self, session_id: str) -> Session:
        resp = await self._client._transport.request("GET", f"{_PATH_SESSIONS}/{session_id}")
        return _coerce_session(resp.data)
    ...
    async def update(
        ...
        resp = await self._client._transport.request("POST", f"{_PATH_SESSIONS}/{session_id}", json=body)
        return _coerce_session(resp.data)
    ...
    async def list(
        ...
        resp = await self._client._transport.request("GET", _PATH_SESSIONS, params=params)
    ...
    async def archive(self, session_id: str) -> Session:
        resp = await self._client._transport.request("POST", f"{_PATH_SESSIONS}/{session_id}/archive")
        return _coerce_session(resp.data)

    async def delete(self, session_id: str) -> DeleteResponse:
        resp = await self._client._transport.request("DELETE", f"{_PATH_SESSIONS}/{session_id}")
        return DeleteResponse(**resp.data)
