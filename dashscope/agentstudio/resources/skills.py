...
from dashscope.agentstudio.types.params import SkillCreateParams, SkillListParams, SkillVersionCreateParams, SkillVersionListParams
...
class Skills:
    ...
    def create(
        ...
        resolved = _resolve_file_id_sync(
            client=self._client,
            file_id=file_id, file=file, mime_type=mime_type,
        )
        body = SkillCreateParams(file_id=resolved).to_dict()
        resp = self._client._transport.request("POST", _PATH_SKILLS, json=body)
        return _coerce_skill(resp.data)

    def retrieve(self, skill_id: str) -> Skill:
        resp = self._client._transport.request("GET", f"{_PATH_SKILLS}/{skill_id}")
        return _coerce_skill(resp.data)
    ...
    def list(
        ...
        params = SkillListParams(source=source, limit=limit, page=page).to_dict()
        resp = self._client._transport.request("GET", _PATH_SKILLS, params=params)
        return build_page(
            payload=resp.data, item_factory=_coerce_skill, request_id=resp.request_id,
            fetch_next=lambda nxt: self.list(
                source=source, limit=limit, page=nxt,
            ),
        )

    def delete(self, skill_id: str) -> DeleteResponse:
        resp = self._client._transport.request("DELETE", f"{_PATH_SKILLS}/{skill_id}")
        return DeleteResponse(**resp.data)


class SkillVersions:
    """Skill version sub-resource — instance methods backed by a shared client."""
    ...
    def create(
        ...
        resp = self._client._transport.request(
            "POST", f"{_PATH_SKILLS}/{skill_id}/versions",
            json=body,
        )
    ...
    def list(
        ...
        resp = self._client._transport.request(
            "GET", f"{_PATH_SKILLS}/{skill_id}/versions", params=params
        )
        return build_page(
            payload=resp.data, item_factory=_coerce_skill_version, request_id=resp.request_id,
            fetch_next=lambda nxt: self.list(
                skill_id, limit=limit, page=nxt,
            ),
        )

    def retrieve(self, skill_id: str, version: str) -> SkillVersion:
        resp = self._client._transport.request(
            "GET", f"{_PATH_SKILLS}/{skill_id}/versions/{version}"
        )
        return _coerce_skill_version(resp.data)
    ...
    def download(self, skill_id: str, version: str) -> Dict[str, Any]:
        resp = self._client._transport.request(
            "GET", f"{_PATH_SKILLS}/{skill_id}/versions/{version}/content"
        )
...
class AsyncSkills:
    ...
    async def create(
        ...
        body = SkillCreateParams(file_id=resolved).to_dict()
        resp = await self._client._transport.request("POST", _PATH_SKILLS, json=body)
        return _coerce_skill(resp.data)

    async def retrieve(self, skill_id: str) -> Skill:
        resp = await self._client._transport.request("GET", f"{_PATH_SKILLS}/{skill_id}")
        return _coerce_skill(resp.data)
    ...
    async def list(
        ...
        params = SkillListParams(source=source, limit=limit, page=page).to_dict()
        resp = await self._client._transport.request("GET", _PATH_SKILLS, params=params)
    ...
    async def delete(self, skill_id: str) -> DeleteResponse:
        resp = await self._client._transport.request("DELETE", f"{_PATH_SKILLS}/{skill_id}")
        return DeleteResponse(**resp.data)


class AsyncSkillVersions:
    """Async skill version sub-resource — instance methods backed by a shared client."""
...
def _resolve_file_id_sync(
    *,
    ...
    uploaded = client.files.upload(
        file, mime_type=mime_type or "application/zip",
    )
    return uploaded.id
