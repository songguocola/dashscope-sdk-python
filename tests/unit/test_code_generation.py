...
class TestCodeGenerationRequest(MockServerBase):
    def test_custom_sample(self, mock_server: MockServer):
        ...
        assert response.output['choices'][0]['content'] == (
            ...
            '            total_size += os.path.getsize(full_path)\n'
            '    return total_size\n```\n\n'
            '函数名为`file_size`，输入参数是给定路径`path`。'
            '函数通过递归遍历给定路径下的所有文件，使用`os.walk`函数'
            '遍历根目录及其子目录下的文件，计算每个文件的大小并累加到'
            '总大小上。最后，返回总大小作为函数的返回值。'
        )
    ...
    def test_nl2code_sample(self, mock_server: MockServer):
        ...
        assert json.dumps(
            ...
            '[{"role": "user", "content": '
            '"计算给定路径下所有文件的总大小"}, '
            '{"role": "attachment", "meta": {"language": "java"}}]'
        )
    ...
    def test_code2comment_sample(self, mock_server: MockServer):
        ...
        assert json.dumps(
            ...
            '[{"role": "user", "content": '
            '"1. 生成中文注释\n2. 仅生成代码部分，不需要额外解释函数功能\n"}, '
            '{"role": "attachment", "meta": {"code": '
            '"\t\t@Override\n\t\tpublic  CancelExportTaskResponse  '
            'cancelExportTask(\n\t\t\t\tCancelExportTask  '
            'cancelExportTask)  {\n\t\t\tAmazonEC2SkeletonInterface  '
            'ec2Service  =  ServiceProvider.getInstance().'
            'getServiceImpl(AmazonEC2SkeletonInterface.class);\n\t\t\t'
            'return  ec2Service.cancelExportTask(cancelExportTask);\n\t\t}", '
            '"language": "java"}}]'
        ).replace('\t', '\\t').replace('\n', '\\n')
        ...
        assert response.output['choices'][0]['content'] == (
            ...
            'public CancelExportTaskResponse cancelExportTask'
            '(CancelExportTask cancelExportTask) {\n\t'
            'AmazonEC2SkeletonInterface ec2Service = '
    ...
    def test_code2explain_sample(self, mock_server: MockServer):
        ...
        assert response.output['choices'][0]['content'] == (
            '这个Java函数是一个覆盖了另一个方法的函数，'
            '名为`getHeaderCacheSize()`。这个方法是从另一个已覆盖的'
            '方法继承过来的。在`@Override`声明中，可以确定这个函数'
            '覆盖了一个其他的函数。这个函数的返回类型是`int`。\n\n'
            '函数内容是：返回0。这个值意味着在`getHeaderCacheSize()`'
            '方法中，不会进行任何处理或更新。因此，返回的`0`值应该是'
            '没有被处理或更新的值。\n\n总的来说，这个函数的作用可能是为了'
            '让另一个方法返回一个预设的值。但是由于`@Override`的提示，'
            '我们无法确定它的真正目的，需要进一步查看代码才能得到更多的信息。'
        )
    ...
    def test_unittest_sample(self, mock_server: MockServer):
        ...
        assert response.output['choices'][0]['content'] == (
            "这个函数用于解析时间戳映射表的输入字符串并返回该映射表的"
            "实例。函数有两个必选参数：typeClass - 用于标识数据类型的"
            "泛型；input - 输入的时间戳映射表字符串。如果typeClass为"
            "null，将抛出IllegalArgumentException异常；如果input为null，"
            "则返回null。函数内部首先检查输入的字符串是否等于\"空字符串\"，"
            "如果是，则直接返回null；如果不是，则创建TimestampMap的实例，"
            "并使用input字符串创建字符串Reader对象。然后使用读取器逐个"
            "字符解析时间戳字符串，并在解析完成后返回相应的TimestampMap"
            "对象。函数的行为取决于传入的时间戳字符串类型。"
        )
        assert response.output['choices'][0]['frame_id'] == 29
        assert response.output['choices'][0]['finish_reason'] == 'stop'
    ...
    def test_codeqa_sample(self, mock_server: MockServer):
        ...
        assert response.output['choices'][0]['content'] == (
            ...
            "class ThreadingHTTPServer(ThreadingMixIn, HTTPServer): \n"
            "    pass\n\n"
            "server = ThreadingHTTPServer(('localhost', self.server_port), "
    ...
    def test_nl2sql_sample(self, mock_server: MockServer):
        response_obj = {
            'output': {
                'choices': [{
                    ...
                    'index': 0,
                    'content': (
                        "SELECT SUM(score) as '小明的总分数' "
                        "FROM student_score WHERE name = '小明';"
                    ),
                    'frame_id': 3,
                }],
            },
        ...
        assert json.dumps(req['input']['message'], ensure_ascii=False) == (
            '[{"role": "user", "content": "小明的总分数是多少"}, '
            '{"role": "attachment", "meta": {"synonym_infos": '
            '{"学生姓名": "姓名|名字|名称", "学生分数": "分数|得分"}, '
            '"recall_infos": [{"content": "student_score.id=\'小明\'", '
            '"score": "0.83"}], "schema_infos": [{"table_id": '
            '"student_score", "table_desc": "学生分数表", "columns": '
            '[{"col_name": "id", "col_caption": "学生id", '
            '"col_desc": "例值为:1,2,3", "col_type": "string"}, '
            '{"col_name": "name", "col_caption": "学生姓名", '
            '"col_desc": "例值为:张三,李四,小明", "col_type": "string"}, '
            '{"col_name": "score", "col_caption": "学生分数", '
            '"col_desc": "例值为:98,100,66", "col_type": "string"}]}]}}]'
        )

        assert response.status_code == HTTPStatus.OK
        assert response.request_id == 'e61a35b7-db6f-90c2-8677-9620ffea63b6'
        assert response.output['choices'][0]['content'] == (
            "SELECT SUM(score) as '小明的总分数' FROM student_score "
            "WHERE name = '小明';"
        )
