...
class EventSourceResponse(StreamResponse):
    ...
    async def prepare(self, request):
        ...
        if not self.prepared:
            ...
            if request.method == "POST":
                ...
                print(f"Request content: {request_str}")
    ...
    async def _ping(self):
        ...
        while True:
            ...
            await self.write(f": ping{self._sep}{self._sep}".encode("utf-8"))
...
def sse_response(
    ...
    *,
    ...
    if not issubclass(response_cls, EventSourceResponse):
        raise TypeError(
            f"response_cls must be subclass of "
    ...
    return _ContextManager(
        sse._prepare(request),  # pylint: disable=protected-access
    )
