...
def phrases(model, phrase_id: str, phrases: dict, page: int, page_size: int,
            delete: bool):
    ...
    if len(phrases) > 0:
        if phrase_id is not None:
            ...
            return AsrPhraseManager.update_phrases(model=model,
                                                   phrase_id=phrase_id,
                                                   phrases=phrases)
        else:
            print('Create phrases -->')
            return AsrPhraseManager.create_phrases(model=model,
                                                   phrases=phrases)
    else:
        if delete:
            ...
            if page is not None and page_size is not None:
                print('List phrases page %d page_size %d -->' %
                      (page, page_size))
                return AsrPhraseManager.list_phrases(page=page,
                                                     page_size=page_size)
...
def test_by_user():
    ...
    if resp.status_code == HTTPStatus.OK:
        ...
        if resp is not None and resp.output is not None:
            ...
            if 'finetuned_output' in output and output[
                    'finetuned_output'] is not None:
            ...
            if 'finetuned_outputs' in output and output[
                    'finetuned_outputs'] is not None:
                outputs = output['finetuned_outputs']
                print('Get %d info from page_no:%d page_size:%d total:%d ->' %
                      (len(outputs), output['page_no'], output['page_size'],
                       output['total']))
                for item in outputs:
        ...
        print('ERROR, status_code:%d, code_message:%s, error_message:%s' %
