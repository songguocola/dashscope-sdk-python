...
class TestConversationRequest(MockServerBase):
    ...
    def test_message_request(self, mock_server: MockServer):
        ...
        resp = Generation.call(model=model,
                               messages=messages,
                               max_tokens=1024,
                               api_protocol='http',
        ...
        assert req['input']['messages'][0] == Message(role=Role.USER,
                                                      content=prompt)
        assert resp.output.text is None
        assert resp.output.choices[0] == Choice(finish_reason='stop',
                                                message={
                                                    'role': 'assistant',
                                                    'content': 'hello world'
                                                })
    ...
    def test_conversation_with_message_and_prompt(self,
    ...
    def test_not_qwen(self, mock_server: MockServer):
        ...
        mock_server.responses.put(
            json.dumps(TestConversationRequest.text_response_obj))
        response = Generation.call(model=Generation.Models.dolly_12b_v2,
                                   prompt=prompt)
