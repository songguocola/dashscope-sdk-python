...
class TestRuns(MockServerBase):
    ...
    def test_create_complicated(self, mock_server: MockServer):
        ...
        metadata = {'key': 'meta'}
        response = Runs.create(thread_id,
    ...
    def test_list(self, mock_server: MockServer):
        ...
        # get assistant id we send.
        req = mock_server.requests.get(block=True)
        assert req == f'/api/v1/threads/{thread_id}/runs?limit=10&order=inc&after=after&before=before'
    ...
    def test_submit_tool_outputs(self, mock_server: MockServer):
        ...
        response = Runs.submit_tool_outputs(thread_id=thread_id,
                                            run_id=run_id,
    ...
    def test_run_required_function_call(self, mock_server: MockServer):
        ...
        req = mock_server.requests.get(block=True)
        assert req['assistant_id'] == assistant_id
        assert response.required_action.submit_tool_outputs.tool_calls[
            0].id == 'call_1'
    ...
    def test_list_run_steps(self, mock_server: MockServer):
        ...
        # get assistant id we send.
        req = mock_server.requests.get(block=True)
        assert req == f'/api/v1/threads/{thread_id}/runs/{run_id}/steps?limit=10&order=inc&after=after&before=before'
        assert len(response.data) == 2
        assert response.data[0].id == 'step_1'
        assert response.data[0].step_details.type == 'message_creation'
        assert response.data[
            0].step_details.message_creation.message_id == 'msg_1'
        ...
        assert response.data[1].step_details.tool_calls[
            0].function.arguments == "{\"left\":87787,\"right\":788988737}"
        assert response.data[1].step_details.tool_calls[
            0].function.output == '789076524'
        assert response.data[1].step_details.tool_calls[
            0].function.name == 'big_add'
    ...
    def test_retrieve_run_steps(self, mock_server: MockServer):
        ...
        response = Steps.retrieve(step_id,
                                  thread_id=thread_id,
                                  run_id=run_id,
                                  limit=10,
                                  order='inc',
        ...
        # get assistant id we send.
        req = mock_server.requests.get(block=True)
        assert req == f'/api/v1/threads/{thread_id}/runs/{run_id}/steps/{step_id}'
        ...
        assert response.step_details.tool_calls[
            0].function.output == '789076524'
