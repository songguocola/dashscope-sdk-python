...
class TestSynthesis(BaseTestEnvironment):
    ...
    def check_result(self, result):
        ...
        assert result.get_response().code is None or len(
            result.get_response().code) == 0
        assert result.get_response().message is None or len(
            result.get_response().message) == 0
                ...
                for word in words_list:
                    ...
                    if 'phonemes' in word:
                        for phoneme in word['phonemes']:
                            print('     %d - %d :  text: %s  tone: %s' %
                                  (phoneme['begin_time'], phoneme['end_time'],
                                   phoneme['text'], phoneme['tone']))
...
def test_by_user():
    ...
    parser.add_argument('--word_timestamp',
                        type=str2bool,
                        default='False',
                        help='run with word_timestamp or not.')
    parser.add_argument('--phoneme_timestamp',
                        type=str2bool,
                        default='False',
    ...
    if first_data_time > 0:
        print('The cost time of first audio data: %6dms' %
