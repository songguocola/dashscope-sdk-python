...
class TestMessages(MockServerBase):
    ...
    def test_update(self, mock_server: MockServer):
        ...
        metadata = {'key': 'value'}
        response = Messages.update(message_id,
                                   thread_id=thread_id,
                                   metadata=metadata,
                                   workspace='111')
        req = mock_server.requests.get(block=True)
        ...
        assert req[
            'path'] == f'/api/v1/threads/{thread_id}/messages/{message_id}'
    ...
    def test_list(self, mock_server: MockServer):
        ...
        response = Messages.list(thread_id,
                                 limit=10,
                                 order='inc',
                                 after='after',
                                 before='before')
        # get assistant id we send.
        req = mock_server.requests.get(block=True)
        assert req == f'/api/v1/threads/{thread_id}/messages?limit=10&order=inc&after=after&before=before'
    ...
    def test_list_message_files(self, mock_server: MockServer):
        ...
        response = Files.list(message_id,
                              thread_id=thread_id,
                              limit=10,
                              order='inc',
                              after='after',
        ...
        # get assistant id we send.
        req = mock_server.requests.get(block=True)
        assert req == f'/api/v1/threads/{thread_id}/messages/{message_id}/files?limit=10&order=inc&after=after&before=before'  # noqa E501
        assert len(response.data) == 2
    ...
    def test_retrieve_message_file(self, mock_server: MockServer):
        ...
        mock_server.responses.put(json.dumps(response_obj))
        thread_id = 'test_thread_id'
        response = Files.retrieve(file_id,
                                  thread_id=thread_id,
                                  message_id=message_id,
                                  limit=10,
                                  order='inc',
        ...
        # get assistant id we send.
        req = mock_server.requests.get(block=True)
        assert req == f'/api/v1/threads/{thread_id}/messages/{message_id}/files/{file_id}'
