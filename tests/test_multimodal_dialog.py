...
from dashscope.multimodal.multimodal_request_params import Upstream, Downstream, ClientInfo, RequestParameters, Device, \
...
class TestCallback(MultiModalCallback):
    ...
    def on_close(self, close_status_code, close_msg):
        logger.info("close with status code: %d, msg: %s" % (close_status_code, close_msg))
...
class TestMultiModalDialog(BaseTestEnvironment):
    ...
    def test_multimodal_dialog_one_turn(self):
        ...
        self.conversation = MultiModalDialog(app_id="",
        ...
        # 首轮进入Listening
        while DialogState.LISTENING is not self.conversation.get_dialog_state():
            time.sleep(0.1)
        self.conversation.request_to_respond("prompt", "今天天气不错", parameters=None)
        # 等待第二轮Listening
        while DialogState.LISTENING is not self.conversation.get_dialog_state():
            time.sleep(0.1)
